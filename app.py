"""
Dashboard Saga — interface e relatórios.
...
dashboard reflete a mudança na hora. atualizar_base.py importa a planilha no MySQL.
...
st.set_page_config(page_title="Saga — Refis para Palhetas",
                   page_icon="📊", layout="wide")

N_MESES = 3
# Identidade visual: PRETO / AZUL / BRANCO (proporção 60/30/10).
AZUL = "#2456A6"          # azul da marca — cor de acento forte (30%)
AZUL_ESCURO = "#16376A"      # azul escuro (variação para séries)
PRETO = "#0D0D0D"           # fundo dominante (60%)
BRANCO = "#F5F5F5"          # texto/acento claro (10%)
ASSETS = Path(__file__).resolve().parent / "assets"
# Paleta dos gráficos: azul em primeiro plano, com tons e branco/cinzas para
# diferenciar séries mantendo a leitura limpa sobre fundo preto.
PALETTE = ["#2456A6", "#769AD3", "#3E70BD", "#1D437A", "#FFFFFF",
           "#C0C0C0", "#16376A", "#8A8A8A", "#A8BFDF", "#E0E0E0"]
...
# --------------------------- Identidade visual ---------------------------
def _injetar_css():
    """Aplica a identidade PRETO/AZUL/BRANCO (60/30/10) sobre o tema escuro."""
    st.markdown(f"""
        <style>
        /* Títulos e cabeçalhos em azul (acento de marca) */
        h1, h2, h3 {{ color: {BRANCO} !important; }}
        .stApp {{ background: #080B10; }}
        [data-testid="stHeader"] {{ background: #080B10; }}
        .block-container {{ padding-top: 2rem; }}
        /* Cartões de KPI: fundo escuro com acento azul à esquerda */
        div[data-testid="stMetric"] {{
            background: linear-gradient(120deg, #101722, #102343); border-left: 5px solid {AZUL};
        ...
        /* Sidebar mais escura, com borda azul sutil */
        section[data-testid="stSidebar"] {{
            background: #080B10; border-right: 1px solid rgba(36,86,166,.35); }}
        /* Divisores em azul translúcido */
        hr {{ border-color: rgba(36,86,166,.45) !important; }}
        /* Item de menu (radio) selecionado destacado em azul */
        section[data-testid="stSidebar"] label[data-baseweb="radio"]:has(input:checked) {{
            color: {AZUL}; font-weight: 700; }}
        </style>
...
def mostrar_logo():
    ...
    presente em todas as páginas. Sem arquivo, cai para o texto 'SAGA'.
    """
    logo = _achar_logo()
    if logo:
        ...
        st.sidebar.markdown(
            ...
            f'font-weight:900;font-size:30px;letter-spacing:2px;color:{AZUL};'
            f'border:3px solid {AZUL};border-radius:8px;padding:2px 12px;'
            f'display:inline-block;margin-bottom:6px;">SAGA</div>',
            unsafe_allow_html=True)
...
def last_n_months(df, n=N_MESES, mes_fim=None):
    ...
    fim = pd.Timestamp(mes_fim if mes_fim is not None else df["mes"].max())
    inicio = (fim.to_period("M") - (n - 1)).start_time
    janela = df[df["mes"].between(inicio, fim)].copy()
    return janela, sorted(janela["mes"].unique())
...
def agg_by(df, group):
    ...
    g = df.groupby(group, as_index=False).agg(
        passagens=("passagens", lambda valores: valores.sum(min_count=1)),
...
def gerar_excel_ranking(f, periodo, filtros_txt):
    ...
    ws.cell(1, 1, "Ranking Vendas Refis para Palhetas — Saga").font = \
...
def pagina_dashboard():
    ...
    if f.empty:
        ...
        return
    if not (f["mes"] == mes_sel).any():
        st.info("Não há dados deste filtro no mês de referência. Selecione outro mês.")
        return
    ...
    colexp.download_button(
        ...
        file_name=f"Ranking_Saga_{dt.date.today().isoformat()}.xlsx",
    ...
    atual = mensal.iloc[-1]
    mes_anterior = (pd.Timestamp(mes_sel).to_period("M") - 1).start_time
    anteriores = mensal[mensal["mes"] == mes_anterior]
    ant = anteriores.iloc[0] if not anteriores.empty else None
    ...
    st.caption("Variação comparada ao mês anterior, quando disponível na base.")
    st.divider()
    ...
    if apu.empty:
        ...
        figu = px.bar(apu, x=xcol, y="unidade", orientation="h",
                      color_discrete_sequence=[AZUL], text=txt)
    ...
    composicao = mensal.melt(id_vars="mes_lbl", value_vars=["total_diant", "total_tras"],
                            var_name="tipo", value_name="faturamento")
    composicao["tipo"] = composicao["tipo"].map({"total_diant": "Dianteiro", "total_tras": "Traseiro"})
    fig = px.bar(composicao, x="mes_lbl", y="faturamento", color="tipo",
                 color_discrete_sequence=PALETTE, barmode="stack", template="plotly_dark",
                 labels={"mes_lbl": "Mês", "faturamento": "Faturamento (R$)", "tipo": "Refil"})
    fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
    st.plotly_chart(fig, width="stretch")

    st.subheader("Fluxo × conversão")
    fluxo = agg_by(f, ["consultor", "unidade"]).dropna(subset=["passagens", "aproveitamento"])
    if not fluxo.empty:
        fig = px.scatter(fluxo, x="passagens", y="aproveitamento", color="unidade",
                         size="total_geral" if fluxo["total_geral"].max() > 0 else None,
                         hover_name="consultor", color_discrete_sequence=PALETTE,
                         template="plotly_dark", labels={"passagens": "Passagens", "aproveitamento": "Aproveitamento", "unidade": "Unidade"})
        fig.update_yaxes(tickformat=".0%")
        fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig, width="stretch")
    else:
        st.caption("Informe passagens para visualizar a conversão dos consultores.")
    ...
    with st.expander("Ver dados detalhados (filtro aplicado)"):
        ...
        show["Aproveitamento"] = show["Aproveitamento"] * 100
...
# as concessionárias, linha a linha por produto) via `atualizar_base.py`.
#
...
def _xlsx_verbas_mes(brutos, ano):
    ...
    cab_fill = PatternFill("solid", fgColor=AZUL.lstrip("#"))
...
def pagina_verbas():
    ...
    if df.empty and not mkt_pagos:
        st.info("A planilha atual não contém verbas ou pagamentos. Os valores serão exibidos quando essa base for fornecida.")
        st.caption("Comissões da Saga ainda não configuradas.")
        return
...
# ========================= ACESSO AO LANÇAMENTO =========================
def _senha_esperada():
    return str(CONFIG.get("acesso", {}).get("senha_lancamento", ""))
...
def _travar_lancamento():
    ...
    for chave in ("lanc_ok", "lanc_visto", "lanc_erros", "lanc_espera", "mostrar_acesso", "erro_acesso"):
...
def _autenticar():
    senha = st.session_state.pop("senha_acesso", "")
    restante = st.session_state.get("lanc_espera", 0.0) - time.monotonic()
    if restante > 0:
        st.session_state["erro_acesso"] = f"Muitas tentativas. Aguarde {int(restante) + 1}s."
        return
    if _senha_correta(senha):
        ...
        for chave in ("lanc_erros", "mostrar_acesso", "erro_acesso"):
            st.session_state.pop(chave, None)
        return
    ...
    if erros >= _MAX_TENTATIVAS:
        ...
        st.session_state["erro_acesso"] = "Muitas tentativas. Aguarde 1 minuto antes de tentar de novo."
    else:
        st.session_state["lanc_erros"] = erros
        st.session_state["erro_acesso"] = f"Senha incorreta. Tentativa {erros} de {_MAX_TENTATIVAS}."


def _dialogo_senha():
    st.caption("Informe a senha para liberar Lançamento e Histórico.")
    if not _senha_esperada():
        st.info("Defina a senha de acesso nas configurações do projeto.")
        return
    with st.form("form_senha_lanc", clear_on_submit=True, border=False):
        st.text_input("Senha", type="password", key="senha_acesso",
                      label_visibility="collapsed", placeholder="Senha")
        st.form_submit_button("Entrar", type="primary", width="stretch", on_click=_autenticar)
    if st.session_state.get("erro_acesso"):
        st.error(st.session_state["erro_acesso"])
...
def _barra_acesso():
    ...
    if st.sidebar.button("🔓" if liberado else "🔒", key="btn_cadeado",
        ...
        if liberado:
            ...
            st.session_state["mostrar_acesso"] = True
    if not liberado and st.session_state.get("mostrar_acesso"):
        with st.sidebar:
...
def pagina_lancamento():
    ...
    if existente:
        st.info(f"Já existe lançamento para **{nome_cons}** em **{label_sel}**. "
                ...
                "em 🗂️ Histórico, então a diferença representa a variação desde a atualização anterior.")
    ...
    chave = f"{uni['id']}_{cons['id']}_{mes.isoformat()}"
    st.divider()
    ...
    preco_d, preco_t = (existente["preco_diant"], existente["preco_tras"]) if existente else (uni["preco_diant"], uni["preco_tras"])
    ...
    st.caption(f"Preços da unidade {uni['nome_exibicao']}: dianteiro {fmt_money(preco_d)} · "
...
def gerar_excel_historico(h, filtros_txt):
    ...
    ws = wb.active
    ws.title = "Histórico"
    ws.cell(1, 1, "Histórico de Lançamentos — Saga").font = \
        Font(name="Arial", bold=True, size=14, color=NAVY)
    ...
    lanc = h  # Inclui os deltas negativos das exclusões para conciliar os totais.
    if not lanc.empty:
...
def pagina_historico():
    ...
    st.caption("Todo lançamento gravado fica registrado aqui, inclusive os que foram "
               ...
               "anterior. A ordem das atualizações não equivale a semanas do calendário.")
    ...
    k4.metric("Faturamento no recorte", fmt_money(f["total_periodo"].sum()))
    ...
    colexp.download_button(
        ...
        file_name=f"Historico_Lancamentos_Saga_{dt.date.today().isoformat()}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        width="stretch",
    )
...
VERBA_GER_DIANT, VERBA_GER_TRAS = float("nan"), float("nan")
...
def _relatorio_semana(df, mes_sel):
    ...
    g["aprov_d"] = g["aproveitamento"]
    g["aprov_t"] = g["aproveitamento_tras"]
    ...
    p = ov["passagens"]
    ov["aprov_d"] = ov["aproveitamento"]
    ov["aprov_t"] = ov["aproveitamento_tras"]
...
def gerar_excel_semanal(g, ov, semana_lbl, ordenar):
    ...
    ws.cell(1, 1, f"Resultado SAGA {semana_lbl}").font = \
...
def pagina_relatorio_semanal():
    ...
    cexp.download_button(
        ...
        file_name=f"Relatorio_Semanal_Saga_{pd.Timestamp(mes_sel).date().isoformat()}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        width="stretch",
    )
    ...
    for i, (_, r) in enumerate(g.iterrows(), 1):
        linhas.append({
            ...
            "Part %": fmt_pct(r["part"]), "Verba": "Não configurada",
        })
    linhas.append({
        ...
        "Part %": "100,0%", "Verba": "Não configurada",
    })
    st.dataframe(pd.DataFrame(linhas), width="stretch", hide_index=True)
    st.caption("Comissões não configuradas. Part % = participação da unidade no faturamento do mês.")


# ===================== PÁGINA: RELATÓRIO POR CONSULTOR =====================
# Verba paga ao consultor por refil vendido (R$ por unidade).
VERBA_DIANT, VERBA_TRAS = float("nan"), float("nan")
...
def gerar_excel_consultor(g, ov, mes_lbl, ordenar):
    ...
    ws.cell(1, 1, f"Resultado por Consultor SAGA {mes_lbl}").font = \
        Font(name="Arial", bold=True, size=13, color=NAVY)
    ws.cell(2, 1, f"Ordenado por {ordenar} · Gerado em "
                  f"{dt.datetime.now().strftime('%d/%m/%Y %H:%M')}").font = \
        Font(name="Arial", size=9, color="FF666666")
...
def pagina_relatorio_consultor():
    ...
    cexp.download_button(
        ...
        file_name=f"Relatorio_Consultor_Saga_{pd.Timestamp(mes_sel).date().isoformat()}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        width="stretch",
    )
    ...
    st.dataframe(pd.DataFrame(linhas), width="stretch", hide_index=True)
    st.caption("Comissões não configuradas. Part % = participação do consultor no faturamento do mês.")
...
if st.sidebar.button("Atualizar visualização", width="stretch"):
    ...
    st.rerun()
try:
    ultima = db.ultima_importacao()
    if ultima:
        st.sidebar.caption(f"Última carga: {ultima['registrado_em']:%d/%m/%Y %H:%M} · {ultima['linhas']} registros")
except Exception:
    pass
